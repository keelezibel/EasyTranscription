...
class Transcribe:
    ...
    def init_whisper_model(self):
        if self.use_gpu:
            ...
            self.whisper_model = Whisper.from_pretrained(os.getenv("WHISPERCPP_MODEL_SIZE"))
    ...
    def run_whisper_cpu(self, out_path):
        print("Running VAD...")
        ...
        for i in range(len(u)):
            for x in range(self.max_attempts):
                result = self.whisper_model.transcribe_from_file(
                    f"{vad_chunks_path}/" + str(i) + ".wav"
                )
    ...
    def iterate_files(self):
        ...
        for audio_path in tqdm(wav_files):
            ...
            if self.use_gpu:
                ...
                out_path = os.path.join(out_path_folder, f"{filename}.txt")
